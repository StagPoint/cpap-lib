...
public class DailyReportViewModel : DailyReport, INotifyPropertyChanged
{
    ...
    public void DeleteSession( Session session )
    {
        ...
        var notes = $"Deleted '{session.Source}' session starting at {session.StartTime:g} and ending at {session.EndTime}.";
        if( eventCount > Events.Count )
        {
            notes += "\n{eventCount - Events.Count} events associated with the session were deleted.";
        }
        AppendNote( $"{notes}\n", false );
