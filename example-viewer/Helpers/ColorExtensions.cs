...
public static class ColorTransforms
{
	...
	public static double[] RgBtoHsl( Color color )
	{
		...
		// hue
		if( Math.Abs( max - min ) < ColorTransforms.tolerance )
			h = 0D; // undefined
		else if( (Math.Abs( max - r ) < ColorTransforms.tolerance)
		         && (g >= b) )
			h = (60D * (g - b)) / (max - min);
		else if( (Math.Abs( max - r ) < ColorTransforms.tolerance)
		         && (g < b) )
			h = ((60D * (g - b)) / (max - min)) + 360D;
		else if( Math.Abs( max - g ) < ColorTransforms.tolerance )
			h = ((60D * (b - r)) / (max - min)) + 120D;
		else if( Math.Abs( max - b ) < ColorTransforms.tolerance )
		...
		// saturation
		if( (Math.Abs( l ) < ColorTransforms.tolerance)
		    || (Math.Abs( max - min ) < ColorTransforms.tolerance) )
			s = 0D;
		else if( (0D < l)
		         && (l <= .5D) )
			s = (max - min) / (max + min);
	...
	public static Color HsLtoRgb( double h, double s, double l, int a = 255 )
	{
		...
		if( Math.Abs( s ) < ColorTransforms.tolerance )
		{
	...
	public static double[] RgBtoHsb( Color color )
	{
		...
		double h = 0D;
		if( (Math.Abs( max - r ) < ColorTransforms.tolerance)
		    && (g >= b) )
			h = (60D * (g - b)) / (max - min);
		else if( (Math.Abs( max - r ) < ColorTransforms.tolerance)
		         && (g < b) )
		...
		double s = Math.Abs( max ) < ColorTransforms.tolerance
			? 0D
			: 1D - (min / max);
		...
		byte?              outputAlpha = null )
	{
		double[] hsl = colorTransformMode == ColorTransformMode.Hsl
			? ColorTransforms.RgBtoHsl( color )
			: ColorTransforms.RgBtoHsb( color );
		if( (Math.Abs( hsl[ 2 ] ) < ColorTransforms.tolerance)
		...
		return colorTransformMode == ColorTransformMode.Hsl
			? ColorTransforms.HsLtoRgb( hsl[ 0 ], hsl[ 1 ], hsl[ 2 ], outputAlpha ?? color.A )
			: ColorTransforms.HsBtoRgb( hsl[ 0 ], hsl[ 1 ], hsl[ 2 ], outputAlpha ?? color.A );
	}
		...
		byte?              outputAlpha = null )
	{
		double[] hsl = colorTransformMode == ColorTransformMode.Hsl
			? ColorTransforms.RgBtoHsl( color )
			: ColorTransforms.RgBtoHsb( color );
		if( (Math.Abs( hsl[ 1 ] ) < ColorTransforms.tolerance)
